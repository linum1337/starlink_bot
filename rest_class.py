...
def tg_id(abonent_id, tg):
    ...
    response = requests.post('http://85.26.197.246:8082/rest_api/v2/AttributeValues/', headers=headers, data=data)
    print(response)
...
def abon_id(login):
    ...
    response = requests.post('http://85.26.197.246:8082/rest_api/v2/Users/', headers=headers, data=data).json()
...
def abon_balance(abon_id):
    ...
    response = requests.post('http://85.26.197.246:8082/rest_api/v2/AdminAccounts/', headers=headers, data=data).json()
...
def abon_tarif(abon_id):
    ...
    response = requests.post('http://85.26.197.246:8082/rest_api/v2/Abonents/', headers=headers, data=data).json()
    tarif_id = str(response['result']['fields']['tarif_id'])
    data_for_tarif = 'method1=objects.get&arg1={"id":"' + str(tarif_id) + '"}&fields=["name"]'
    response_for_tarif = requests.post('http://85.26.197.246:8082/rest_api/v2/Tarif/', headers=headers, data=data_for_tarif).json()
...
def abon_usluga(abon_id):
    ...
    response = requests.post('http://85.26.197.246:8082/rest_api/v2/UsersUsluga/', headers=headers, data=data).json()
    uslugi = []
